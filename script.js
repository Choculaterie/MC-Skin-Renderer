...
async function fetchSkinFromMojang(usernameOrUUID) {
    ...
    try {
        ...
        if (!input.match(/^[0-9a-f]{32}$/i) && !input.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
            ...
            const profileResponse = await fetch(`https://backend.choculaterie.com/api/MojangProxy?endpoint=${encodeURIComponent(mojangUrl)}`);
        ...
        const sessionResponse = await fetch(`https://backend.choculaterie.com/api/MojangProxy?endpoint=${encodeURIComponent(sessionUrl)}`);
